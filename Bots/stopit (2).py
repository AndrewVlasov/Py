...
def start_timer(message,connect, cur):
    ...
    while True:
        try:
            ...
            with connect:
                cur = connect.cursor()
                sql = "INSERT INTO bot_db.dontdoit (user_id, username, first_name, last_name, chat_id, reply_to_msg_id, what, active, TOKEN)" \
...
def restart_timer(user_id, connect, cur):
    while True:
        try:
            print("Restart")
            with connect:
                sql_update_query = """update bot_db.dontdoit set started_date = current_timestamp() where user_id = (%s) and active = 1"""
                val = user_id
                cur.execute(sql_update_query, val)
            # bot.send_message(user_id, 'Ну это ты зря. Таймер перезапущен');
